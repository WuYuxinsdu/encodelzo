...
/*
 * 32-bit integer manipulation macros (big endian)   //32位整数操作宏（大端）
 */
...
/*
 * DES key schedule (56-bit, encryption)   //DES密钥一览表（56位加密）
 */
int des_setkey_enc( des_context *ctx, const unsigned char key[DES_KEY_SIZE] )  //定义DES_KEY_SIZE为8
{
    des_setkey( ctx->sk, key ); //子密钥的产生
...
/*
 * DES-ECB block encryption/decryption  //DES-ECB块加密/解密
 */
 ...
 */
unsigned int des_ecb_encrypt(unsigned char *pout,   //数据加密后存放的位置
                             unsigned char *pdata,  //源字符串存放的位置
                             unsigned int nlen,     //源字符串的长度
                             unsigned char *pkey)   //初始密钥
{
    unsigned char *tmp;
    unsigned int len,i;
    unsigned char ch = '\0';
    des_context ctx;   //定义ctx为结构体，存放子密钥

    des_setkey_enc( &ctx, pkey );//根据输入的初始密钥，产生相关的子密钥
    ...
    for(i = 0;i < nlen;i += 8)
    {
        des_crypt_ecb( &ctx, (pdata + i), (pout + i) );  //DES_ECB模式加密
    }
    if(len > nlen)  //说明源字符串的长度不是8的整数倍
    {
        tmp = (unsigned char *)malloc(len);
        i -= 8;
        memcpy(tmp,pdata + i,nlen - i);   //从pdata + i的开始位置拷贝nlen - i个字节的数据到tmp。
        memset(tmp + nlen % 8, ch, (8 - nlen % 8) % 8);//将tmp + nlen % 8中前(8 - nlen % 8) % 8个字节用 ch 替换并返回tmp + nlen % 8
 ...
 */
unsigned int des_ecb_decrypt(unsigned char *pout,  //解密后输出的字符串
                             unsigned char *pdata,
                             unsigned int nlen,
                             unsigned char *pkey)
{
                             ...
                             unsigned char *piv)
{
    ...
    for(i = 0;i < nlen;i += 8)
    {
        des_crypt_cbc( &ctx, 1, 8, pivb, (pdata + i), (pout + i));
    }
    if(len > nlen)
    {
        tmp = (unsigned char *)malloc(len);
        ...
        des_crypt_cbc( &ctx, 1, 8, pivb, tmp, (pout + i +8));
        free(tmp);
    }
                             ...
                             unsigned char *piv)
{
    ...
    for(i = 0;i < nlen;i += 8)
    {
        des_crypt_cbc( &ctx, 0, 8, pivb, (pdata + i), (pout + i));
    }
